...
# Controlled Windows attacker simulation
# All artifacts created by this script are removed after execution.
...
try {
    ...
    $ts = Get-UtcTimestamp
    Add-Action 2 `
        "Add support_update to local Administrators group" `
        $ts `
        "Security Event ID 4732; Sysmon Event ID 1" `
    ...
    $ts = Get-UtcTimestamp
    Add-Action 5 `
        "Initiate outbound TCP connection to safe external IP 1.1.1.1:443" `
        $ts `
        "Sysmon Event ID 3" `
