...
$hourBuckets = $times | ForEach-Object {
    $_.ToString("yyyy-MM-dd HH:00")
} | Group-Object
...
$largestGap = if ($gaps.Count -gt 0) {
    [math]::Round(($gaps | Measure-Object -Maximum).Maximum,0)
}
else {
    0
}
...
$cmdComplete = if ($procEvents.Count -gt 0) {
    [math]::Round((($procEvents | Where-Object {$_.command_line}).Count/$procEvents.Count)*100,1)
}
else {
    100
}
...
$ipComplete = if ($logonEvents.Count -gt 0) {
    [math]::Round((($logonEvents | Where-Object {$_.source_ip}).Count/$logonEvents.Count)*100,1)
}
else {
    100
}
...
$sbComplete = if ($psEvents.Count -gt 0) {
    [math]::Round((($psEvents | Where-Object {$_.decoded_script_block}).Count/$psEvents.Count)*100,1)
}
else {
    100
}
...
$score = [math]::Round(
    (($cmdComplete*0.3)+($ipComplete*0.3)+($sbComplete*0.2)+($hourScore*0.2)),
    1
)

$assessment = if ($score -ge 90) {
    "good"
}
elseif ($score -ge 70) {
    "acceptable"
...
Write-Host "Hours without events: $hoursWithoutEvents"
