...
Write-Host "    [2/5] Encoded command, decoded ScriptBlock content..."
$s2 = Get-Date
...
if ($e4) { Write-Host "          EID 4104: full multi-line ScriptBlock captured        [PASS]" }
else { Write-Host "          EID 4104: full multi-line ScriptBlock NOT captured        [FAIL]" }
...
if ($e5) { Write-Host "          C:\PSTranscripts\*.txt exists, transcript recorded    [PASS]" }
