...
auditctl -w /etc/crontab -p r -k audit_test_cron 2>/dev/null || true; cat /etc/crontab >/dev/null 2>&1; c=$(chk audit_test_cron); [[ $c -gt 0 ]] && s=CAPTURED && CAPTURED=$((CAPTURED+1)) || s=MISSED; rec 6 "cron configuration check" audit_test_cron "cat /etc/crontab" "$s" "$c"
