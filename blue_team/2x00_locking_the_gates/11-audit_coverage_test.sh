...
wget --version >/dev/null 2>&1 || true; curl --version >/dev/null 2>&1 || true; c=$(chk suspicious_download); [[ $c -gt 0 ]] && s=CAPTURED && CAPTURED=$((CAPTURED+1)) || s=MISSED; rec 3 "suspicious download tool" suspicious_download "wget/curl" "$s" "$c"
