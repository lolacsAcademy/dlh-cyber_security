...
if ($pw.MinPasswordLength -lt 14) {
    Add "CRITICAL" "Password Policy" "Domain" "Minimum length: $($pw.MinPasswordLength)" "Weak passwords" "Set to 14" "Task 4"
}
...
if ($pne.Count -gt 0) {
    Add "HIGH" "Password Policy" "$($pne.Count) accounts" "PasswordNeverExpires" "Never rotates" "Disable flag" "Task 4"
}
...
if ($disabledPriv.Count -gt 0) {
    Add "HIGH" "Privileged Accounts" "$($disabledPriv.Count) accounts" "Disabled but privileged" "Stale attack surface" "Remove from group" "Task 4"
}

if ($stale.Count -gt 0) {
    Add "MEDIUM" "Stale Objects" "$($stale.Count) computers" "No logon 90+ days" "Unmaintained risk" "Disable objects" "Task 6"
}

if (@($gpos | Where-Object {$_.DisplayName -like "*MedDefense*"}).Count -eq 0) {
    Add "MEDIUM" "GPO Posture" "Domain" "No MedDefense GPOs" "No baseline" "Create GPOs" "Task 8"
}
