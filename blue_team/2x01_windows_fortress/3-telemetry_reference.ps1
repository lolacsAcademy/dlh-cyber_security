...
$events = @(
    ...
    [PSCustomObject]@{event_id="4688";event_name="Process Creation";log_source="Security";audit_or_sensor_dependency="Process Tracking subcategory";security_meaning="New process launched";normal_frequency="High";triage_priority="Medium";crimson_tide_phase="Execution";example_suspicious_pattern="encoded PowerShell command with suspicious flags";validation_method="Get-WinEvent -Id 4688"}
    ...
    [PSCustomObject]@{event_id="4104";event_name="Script Block Logging";log_source="PowerShell";audit_or_sensor_dependency="Script Block Logging enabled";security_meaning="Full script content logged";normal_frequency="Medium";triage_priority="High";crimson_tide_phase="Execution";example_suspicious_pattern="obfuscated or base64 encoded script block";validation_method="Get-WinEvent -Id 4104"}
    [PSCustomObject]@{event_id="1";event_name="Process Creation";log_source="Sysmon";audit_or_sensor_dependency="Sysmon installed";security_meaning="Detailed process creation";normal_frequency="High";triage_priority="Medium";crimson_tide_phase="Execution";example_suspicious_pattern="unsigned binary launched from Temp folder";validation_method="Get-WinEvent -LogName Microsoft-Windows-Sysmon/Operational -Id 1"}
    ...
    [PSCustomObject]@{event_id="7";event_name="Image Loaded";log_source="Sysmon";audit_or_sensor_dependency="Sysmon installed";security_meaning="DLL loaded into process";normal_frequency="High";triage_priority="Low";crimson_tide_phase="Execution";example_suspicious_pattern="unsigned DLL loaded into lsass.exe";validation_method="Get-WinEvent -LogName Microsoft-Windows-Sysmon/Operational -Id 7"}
    [PSCustomObject]@{event_id="11";event_name="File Created";log_source="Sysmon";audit_or_sensor_dependency="Sysmon installed";security_meaning="File written to disk";normal_frequency="High";triage_priority="Medium";crimson_tide_phase="Impact";example_suspicious_pattern="mass file creation with ransomware extension";validation_method="Get-WinEvent -LogName Microsoft-Windows-Sysmon/Operational -Id 11"}
    [PSCustomObject]@{event_id="13";event_name="Registry Value Set";log_source="Sysmon";audit_or_sensor_dependency="Sysmon installed";security_meaning="Registry value modified";normal_frequency="Medium";triage_priority="Medium";crimson_tide_phase="Persistence";example_suspicious_pattern="run key added for autostart persistence";validation_method="Get-WinEvent -LogName Microsoft-Windows-Sysmon/Operational -Id 13"}
    [PSCustomObject]@{event_id="22";event_name="DNS Query";log_source="Sysmon";audit_or_sensor_dependency="Sysmon installed";security_meaning="DNS resolution requested";normal_frequency="High";triage_priority="Medium";crimson_tide_phase="Command and Control";example_suspicious_pattern="query to known malicious domain";validation_method="Get-WinEvent -LogName Microsoft-Windows-Sysmon/Operational -Id 22"}
)
