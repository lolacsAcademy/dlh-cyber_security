...
$CurrentEnc = Get-ItemProperty -Path $KdcPath -Name $KerberosValueName -ErrorAction SilentlyContinue
...
$ServiceAccounts = @(Get-ADUser -LDAPFilter "(servicePrincipalName=*)" -Properties ServicePrincipalName,UseDESKeyOnly,msDS-SupportedEncryptionTypes)
$DESAccounts = @($ServiceAccounts | Where-Object { $_.UseDESKeyOnly -eq $true })
...
foreach ($Account in $DESAccounts) {
    ...
    Set-ADAccountControl -Identity $Account.DistinguishedName -UseDESKeyOnly $false
    Write-Host "              [DONE]"
}
...
# Credential Guard status is indicated by LsaCfgFlags registry value
...
$VerifiedKerberos = (Get-ItemProperty -Path $KdcPath -Name "DefaultDomainSupportedEncTypes").DefaultDomainSupportedEncTypes
...
$VerifiedNTLM = (Get-ItemProperty -Path $LsaPath -Name "LmCompatibilityLevel").LmCompatibilityLevel
...
$RemainingDES = @(Get-ADUser -Filter * -Properties UseDESKeyOnly | Where-Object { $_.UseDESKeyOnly -eq $true })
