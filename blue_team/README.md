# Blue Team

Security posture assessment project for MedDefense Health Systems (fictional scenario), completed as part of the Defensive Cybersecurity specialisation.

## Tasks

- Task 0: 1x00_first_watch/0-environment_summary.md
- Task 1: 1x00_first_watch/1-incident_classification.md
- Task 2: 1x00_first_watch/2-root_cause_analysis.md
- Task 3: 1x00_first_watch/3-physical_assessment.md
- Task 4: 1x00_first_watch/4-control_inventory.md
- Task 5: 1x00_first_watch/5-control_gaps.md
- Task 6: 1x00_first_watch/6-compensating_controls.md
- Task 7: 1x00_first_watch/7-asset_registry.md
- Task 8: 1x00_first_watch/8-criticality_assessment.md
- Task 9: 1x00_first_watch/9-data_map.md
- Task 10: 1x00_first_watch/10-complete_control_matrix.md
- Task 11: 1x00_first_watch/11-shadow_systems.md
- Task 12: 1x00_first_watch/12-gap_analysis.md
