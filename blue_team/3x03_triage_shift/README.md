Triage Shift
Task 0   0-queue_assessment.sh, queue_assessment.json
Task 1   triage_methodology.md
Task 2   2-context_assembly.sh, enriched_queue.json
Task 3   3-triage_clearcut_tp.sh, tickets/batch1_clearcut_tp.json
Task 4   4-triage_clearcut_fp.sh, tickets/batch2_clearcut_fp.json
Task 5   6-triage_ambiguous_auth.sh, tickets/batch4_auth.json
Task 6   7-triage_ambiguous_proc_net.sh, tickets/batch5_proc_net.json
